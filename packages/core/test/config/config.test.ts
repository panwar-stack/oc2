...
import { ConfigProvider } from "@oc2-ai/core/config/provider"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "oc2.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "oc2.jsonc"),
                `{
  ...
  it.live("loads global, ancestor, and .oc2 configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".oc2"), { recursive: true })
            await fs.mkdir(path.join(directory, ".oc2"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "oc2.json"), JSON.stringify({ $schema: "outside" })),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".oc2")),
              AbsolutePath.make(path.join(directory, ".oc2")),
            ])
