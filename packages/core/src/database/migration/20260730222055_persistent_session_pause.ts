...
export default {
  id: "20260730222055_persistent_session_pause",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(`CREATE INDEX \`session_pause_blocker_cascade_idx\` ON \`session_pause_blocker\` (\`cascade_id\`);`)
      yield* tx.run(
        `CREATE UNIQUE INDEX \`session_pause_cascade_root_generation_idx\` ON \`session_pause_cascade\` (\`root_session_id\`,\`generation\`);`,
      )
      yield* tx.run(
