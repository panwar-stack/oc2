...
Supervisor session endpoints are exposed by the v2 SDK under `client.supervisor`.

| Method | Description | Response |
| ------ | ----------- | -------- |
| `supervisor.get({ sessionID })` | Get the effective supervisor config and current state for a session | <a href={typesV2Url}><code>SupervisorState</code></a> |
| `supervisor.update({ sessionID, supervisorSettingsPatch })` | Update durable per-session supervisor overrides | <a href={typesV2Url}><code>SupervisorState</code></a> |
| `supervisor.report({ sessionID })` | Generate and return a supervisor report | <a href={typesV2Url}><code>SupervisorReport</code></a> |
...
// Enable advisory supervision for this session
await client.supervisor.update({
  sessionID: session.id,
  supervisorSettingsPatch: { mode: "advise", insert_recommendations: true },
})
```
...
Supervisor events include `supervisor.state.updated`, `supervisor.recommendation.created`, `supervisor.settings.updated`, and `supervisor.report.completed`.
