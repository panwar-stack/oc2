---
title: SDK
description: Type-safe JS client for oc2 server.
---
...
The oc2 JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control oc2 programmatically.
...
Create an instance of oc2:

```javascript
import { createOc2 } from "@oc2-ai/sdk"

const { client } = await createOc2()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `oc2.json`, but you can override or add configuration inline:

```javascript
import { createOc2 } from "@oc2-ai/sdk"

const oc2 = await createOc2({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${oc2.server.url}`)
...
If you already have a running instance of oc2, you can create a client instance to connect to it:
...
The legacy `createOpencode` and `createOpencodeClient` exports remain available for compatibility.
...
```typescript
import type { Session, Message, Part } from "@oc2-ai/sdk"
```
...
| `session.init({ path, body })`                             | Run `/init` to create or update `AGENTS.md` | Returns `boolean`                                                                                                                                                                                                        |
...
```javascript
import { createOc2Client } from "@oc2-ai/sdk/v2"
