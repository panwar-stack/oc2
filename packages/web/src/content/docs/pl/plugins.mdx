---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć oc2.
---

Wtyczki umożliwiające wyłączenie oc2 poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania oc2.
...
- `.oc2/plugins/` - Wtyczki na poziomie projektu
- `~/.config/oc2/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/oc2/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/oc2/oc2.json`)
2. Konfiguracja projektu (`oc2.json`)
3. Globalny katalog wtyczek (`~/.config/oc2/plugins/`)
4. Katalog wtyczek projektu (`.oc2/plugins/`)
...
oc2 uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z oc2 do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do oc2.
...
:::note
Jeśli korzystasz z aplikacji komputerowej oc2, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj oc2 czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do oc2:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod oc2. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://oc2.ai/docs/sdk).
