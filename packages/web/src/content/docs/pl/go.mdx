...
OC2 Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w OC2. Subskrybujesz OC2 Go i
...
OC2 Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

OC2 Go działa jak każdy inny dostawca w OC2.

1. Zaloguj się do **<a href={console}>OC2 Zen</a>**, subskrybuj Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `OC2 Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować OC2 Go.
:::
...
OC2 Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji OC2
używa formatu `oc2-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`oc2-go/kimi-k2.6` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z OC2.
