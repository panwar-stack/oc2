...
Aby debugować problemy z oc2, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\oc2\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `oc2 --log-level DEBUG`.

---

## Przechowywanie danych

oc2 przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/oc2/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\oc2`
...
oc2 Desktop uruchamia w tle lokalny serwer oc2 (sidecar `oc2-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OC2` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/oc2/oc2.jsonc` (lub `~/.config/oc2/oc2.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/oc2/oc2.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\oc2\oc2.jsonc`
...
oc2 może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/oc2/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\oc2\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby oc2 mógł ją odbudować.

1. Całkowicie zamknij oc2 Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/oc2`
- **Linux**: usuń `~/.cache/oc2` (lub uruchom `rm -rf ~/.cache/oc2`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\oc2`

3. Uruchom ponownie oc2 Desktop.

---

### Napraw problemy z połączeniem z serwerem

oc2 Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `oc2.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OC2_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OC2_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows oc2 Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami oc2.

---

### Brak powiadomień

oc2 Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla oc2 w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie oc2 Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji oc2 Desktop):

- `oc2.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `oc2.global.dat` i `oc2.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `oc2 upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `oc2 models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\oc2`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. oc2 dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\oc2`

2. Uruchom ponownie kod oc2, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to oc2 do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
oc2 wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
