...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) oc2, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.oc2/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/oc2/tools/`.
