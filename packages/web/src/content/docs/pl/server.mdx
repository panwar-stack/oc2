---
title: Serwer
description: Interakcja z serwerem oc2 za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `oc2 serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient oc2.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `oc2.local` |
...
Ustaw `OC2_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `oc2` lub ustaw `OC2_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `oc2 serve`, jak i `oc2 web`.
...
Kiedy uruchomisz `oc2`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera oc2 do programowej interakcji z oc2.
:::

Ta architektura umożliwia obsługę wielu klientów przez oc2 i programową interakcję z kodem otwartym.

Możesz uruchomić `oc2 serve`, aby uruchomić samodzielny serwer. Jeśli masz
oc2 TUI działa, `oc2 serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki oc2 [IDE](/docs/ide).
...
Serwer oc2 udostępnia następujące interfejsy API.
