...
Dzięki oc2 możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie oc2 używa naszego własnego motywu `oc2`.
...
oc2 ma kilka wbudowanych motywów.
...
- Chcą, aby OC2 pasował do wyglądu ich terminala
...
oc2 obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/oc2/themes/*.json` lub `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.oc2/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.oc2/themes/*.json`
