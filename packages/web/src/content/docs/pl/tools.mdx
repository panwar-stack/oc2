...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OC2 posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OC2.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OC2_EXPERIMENTAL_LSP_TOOL=true` (lub `OC2_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OC2 lub gdy zmienna środowiskowa `OC2_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OC2:
