---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w oc2.
---

import config from "../../../../config.mjs"
export const console = config.console

oc2 używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji oc2.
...
w `~/.local/share/oc2/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim oc2
config.
...
OC2 Zen to lista modeli dostarczonych przez zespół oc2, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z oc2. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OC2 Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OC2 Zen` i przejdź do [oc2.ai/auth](https://oc2.ai/zen).
...
Działa jak każdy inny dostawca w oc2 i jest całkowicie opcjonalny w użyciu.

---

## OC2 Go

OC2 Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół oc2, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z oc2.

1. Uruchom polecenie `/connect` w TUI, wybierz `OC2 Go` i przejdź do [oc2.ai/auth](https://oc2.ai/zen).
...
Aby używać Amazon Bedrock z oc2:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania oc2:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `oc2.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w oc2 nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować oc2 do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod oc2 działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji oc2.
...
oc2 używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować oc2
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`oc2.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem oc2:
...
Aby używać Google Vertex AI z oc2:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania oc2.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą oc2, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację oc2 jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja oc2 będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować oc2 tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować oc2 tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować oc2 tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem oc2. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z oc2:
...
3. Skopiuj klucz API do użycia w oc2.
...
6. **Ważne**: Przed użyciem modeli chmurowych w oc2 musisz pobrać informacje o modelu lokalnie:
...
OC2 Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół oc2. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OC2 Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OC2 Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API oc2.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji oc2.
...
5. Możesz je także dostosować za pomocą konfiguracji oc2. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z oc2:
...
Możesz także dostosować modele za pomocą konfiguracji oc2. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z oc2. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `oc2.json` w katalogu projektu:
...
Pola `limit` pozwalają oc2 zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `oc2 auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację oc2 i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji oc2.
