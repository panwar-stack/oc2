...
Możesz dodać dodatkowe informacje do oc2 za pomocą _Model Context Protocol_, w skrócie MCP. oc2 obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [oc2 Config](https://oc2.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/oc2`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
oc2 automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, oc2:
...
Jeśli serwer wymaga uwierzytelnienia, oc2 poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `oc2 mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji oc2 bezpieczne przechowa tokeny w `~/.local/share/oc2/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w oc2, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie oc2, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć oc2 z kontem Sentry.
