...
Umiejętności agenta pozwalają oc2 odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
oc2 przeszukuje te lokalizacje:

- Project config: `.oc2/skills/<name>/SKILL.md`
- Global config: `~/.config/oc2/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu oc2 przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.oc2/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/oc2/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.oc2/skills/git-release/SKILL.md` w ten sposób:
...
oc2 wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `oc2.json`:
...
**Dla agentów wbudowanych** (w `oc2.json`):
