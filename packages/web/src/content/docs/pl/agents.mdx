...
W OC2 obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OC2 zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OC2 ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

OC2 ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej OC2, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `oc2.json`:
...
- Globalnie: `~/.config/oc2/agents/`
- W projekcie: `.oc2/agents/`
...
Jeśli nie zostanie skonfigurowana, OC2 używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OC2, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OC2 w `provider/model-id`. Na przykład, [OC2 Zen](/docs/zen), użyj `oc2/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `oc2 models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/panwar-stack/oc2).
:::
