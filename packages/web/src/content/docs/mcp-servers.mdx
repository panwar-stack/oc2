...
You can add external tools to OC2 using the _Model Context Protocol_, or MCP. OC2 supports both local and remote servers.
...
You can define MCP servers in your [OC2 Config](https://oc2.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/oc2` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OC2 automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OC2 will:
...
If the server requires authentication, OC2 will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `oc2 mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OC2 will store the tokens securely in `~/.local/share/oc2/mcp-auth.json`.
...
Your MCPs are available as tools in OC2, alongside built-in tools. So you can manage them through the OC2 config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OC2 to your Sentry account.
