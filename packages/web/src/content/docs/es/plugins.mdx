---
title: Complementos
description: Escriba sus propios complementos para extender OC2.
---

Los complementos le permiten extender OC2 al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OC2.
...
- `.oc2/plugins/` - Complementos a nivel de proyecto
- `~/.config/oc2/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/oc2/node_modules/`.
...
1. Configuración global (`~/.config/oc2/oc2.json`)
2. Configuración del proyecto (`oc2.json`)
3. Directorio global de complementos (`~/.config/oc2/plugins/`)
4. Directorio de complementos del proyecto (`.oc2/plugins/`)
...
OC2 ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK oc2 para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar oc2.
...
:::note
Si está utilizando la aplicación de escritorio OC2, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que oc2 lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a oc2:
...
El ayudante `tool` crea una herramienta personalizada a la que oc2 puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para oc2 junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://oc2.ai/docs/sdk) para obtener más detalles.
