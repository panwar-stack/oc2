...
Las herramientas permiten que LLM realice acciones en su código base. OC2 viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OC2.
...
:::note
Esta herramienta solo está disponible cuando `OC2_EXPERIMENTAL_LSP_TOOL=true` (o `OC2_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OC2 o cuando la variable de entorno `OC2_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OC2:
