---
title: Servidor
description: Interactuar con el servidor oc2 a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `oc2 serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente oc2 puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `oc2.local` |
...
Configure `OC2_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `oc2`, o configure `OC2_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `oc2 serve` como a `oc2 web`.
...
Cuando ejecuta `oc2`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor oc2 para interactuar con oc2 mediante programación.
:::

Esta arquitectura permite que oc2 admita múltiples clientes y le permite interactuar con oc2 mediante programación.

Puede ejecutar `oc2 serve` para iniciar un servidor independiente. Si tienes el
oc2 TUI ejecutándose, `oc2 serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OC2 [IDE](/docs/ide).
...
El servidor oc2 expone las siguientes API.
