...
Con OC2 puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OC2 usa nuestro propio tema `oc2`.
...
OC2 viene con varios temas integrados.
...
- Quiere que OC2 coincida con la apariencia de su terminal
...
OC2 admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/oc2/themes/*.json` o `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.oc2/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.oc2/themes/*.json`
