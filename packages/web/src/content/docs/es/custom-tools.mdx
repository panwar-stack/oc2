...
description: Cree herramientas que LLM pueda llamar en oc2.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de oc2 como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.oc2/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/oc2/tools/`.
...
```ts title=".oc2/tools/database.ts" {1}
...
```ts title=".oc2/tools/project.ts" {8}
import { tool } from "@oc2-ai/plugin"
...
```ts title=".oc2/tools/python-add.ts" {10}
import { tool } from "@oc2-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".oc2/tools/add.py")
