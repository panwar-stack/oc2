...
Puede agregar herramientas externas a OC2 utilizando el _Model Context Protocol_ o MCP. OC2 admite servidores locales y remotos.
...
Puede definir servidores MCP en su [OC2 Config](https://oc2.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/oc2`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
OC2 maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, OC2:
...
Si el servidor requiere autenticación, OC2 le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `oc2 mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, OC2 almacenará los tokens de forma segura en `~/.local/share/oc2/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en OC2, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración OC2 como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar OC2 a su cuenta Sentry.
