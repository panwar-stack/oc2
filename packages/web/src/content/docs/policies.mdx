---
title: Policies
description: Control which configured resources OC2 may use.
---

Policies control whether OC2 may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `oc2.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether OC2 may use a resource such as an LLM provider.
...
OC2 currently supports one policy action:
