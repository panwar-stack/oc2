---
title: TUI
description: Using the OC2 terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OC2 provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OC2 starts the TUI for the current directory.
...
When using the OC2 TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OC2. Allows you to select from available providers and add their API keys.
...
Guided setup for creating or updating `AGENTS.md` with required engineering principles. [Learn more](/docs/rules).
...
Run a named [`local_fusion` config](/docs/config#local-fusion) against a prompt.
...
The first argument is the config name; the rest of the line is the prompt.
...
This is separate from `oc2.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OC2 waits after the leader key. Defaults to `2000`.
...
Use `OC2_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `oc2.default`.
