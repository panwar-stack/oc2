---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OC2.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OC2 fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OC2 avvia la TUI per la directory corrente.
...
Quando usi la TUI di OC2, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OC2. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `oc2.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo OC2 attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `oc2.default`.
