...
In OC2 esistono due tipi di agenti: agenti primari e subagenti.
...
OC2 include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OC2 include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

OC2 include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di OC2, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `oc2.json`:
...
- Globale: `~/.config/oc2/agents/`
- Per progetto: `.oc2/agents/`
...
Se non specifichi una temperature, OC2 usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OC2 sia per la config specifica del progetto.
...
L'ID modello nella configurazione OC2 usa il formato `provider/model-id`. Per esempio, se stai usando [OC2 Zen](/docs/zen), useresti `oc2/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `oc2 models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/panwar-stack/oc2).
:::
