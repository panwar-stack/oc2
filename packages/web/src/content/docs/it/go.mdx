...
OC2 Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in OC2. Ti abboni a OC2 Go e
...
OC2 Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

OC2 Go funziona come qualsiasi altro provider in OC2.

1. Accedi a **<a href={console}>OC2 Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `OC2 Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a OC2 Go.
:::
...
OC2 Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua OC2 config
utilizza il formato `oc2-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`oc2-go/kimi-k2.6` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con OC2.
