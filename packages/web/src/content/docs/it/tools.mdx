...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. OC2 include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in OC2.
...
:::note
Questo strumento e' disponibile solo quando `OC2_EXPERIMENTAL_LSP_TOOL=true` (oppure `OC2_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OC2 o quando la variabile d'ambiente `OC2_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di OC2:
