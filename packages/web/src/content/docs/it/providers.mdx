---
title: Provider
description: Usa qualsiasi provider LLM in OC2.
---

import config from "../../../../config.mjs"
export const console = config.console

OC2 usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OC2.
...
OC2 Zen e una lista di modelli forniti dal team OC2, testati e
verificati per funzionare bene con OC2. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OC2 Zen.
:::

1. Esegui `/connect` nella TUI, seleziona oc2 e vai su [oc2.ai/auth](https://oc2.ai/auth).
...
Funziona come qualsiasi altro provider in OC2 ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OC2:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui oc2:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `oc2.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OC2 non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare oc2 per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché oc2 funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di oc2.
...
salvare il token nello storage di auth di oc2.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
OC2 usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OC2
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `oc2.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Per usare il tuo abbonamento GitHub Copilot con oc2:
...
Per usare Google Vertex AI con OC2:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da oc2, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di OC2 come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OC2 elencata come sessione separata.
...
Puoi configurare oc2 per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare oc2 per usare modelli locali tramite LM Studio.
...
Puoi configurare oc2 per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OC2. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con OC2:
...
3. Copia la chiave API da usare in OC2.
...
6. **Importante**: Prima di usare modelli cloud in OC2, devi scaricare le informazioni del modello localmente:
...
OC2 Zen è una lista di modelli testati e verificati forniti dal team OC2. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OC2 Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OC2 Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OC2.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di oc2.
...
5. Puoi anche personalizzarli tramite la configurazione di oc2. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di oc2. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con oc2. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `oc2.json` nella directory del progetto:
...
I campi `limit` permettono a OC2 di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `oc2 auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di oc2 e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config oc2.
