---
title: Server
description: Interagisci con il server oc2 via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `oc2 serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client oc2.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `oc2.local` |
...
Imposta `OC2_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `oc2`, oppure imposta `OC2_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `oc2 serve` sia per `oc2 web`.
...
Quando esegui `oc2` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server oc2 per interagire con oc2 in modo programmatico.
:::

Questa architettura permette a oc2 di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `oc2 serve` per avviare un server standalone. Se la TUI di oc2 e gia in esecuzione, `oc2 serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OC2.
...
Il server oc2 espone le seguenti API.
