...
Per diagnosticare problemi con OC2, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\oc2\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `oc2 --log-level DEBUG`.

---

## Archiviazione

oc2 salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/oc2/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\oc2`
...
OC2 Desktop esegue in background un server locale di OC2 (il sidecar `oc2-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `OC2` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/oc2/oc2.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\oc2\oc2.jsonc`
...
OC2 puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/oc2/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\oc2\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che OC2 possa ricostruirla.

1. Chiudi completamente OC2 Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/oc2`
- **Linux**: elimina `~/.cache/oc2` (oppure esegui `rm -rf ~/.cache/oc2`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\oc2`

3. Riavvia OC2 Desktop.

---

### Risolvi problemi di connessione al server

OC2 Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `oc2.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OC2_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OC2_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, OC2 Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di OC2.

---

### Notifiche non visualizzate

OC2 Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per OC2 nelle impostazioni del sistema operativo, e
...
1. Chiudi OC2 Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app OC2 Desktop):

- `oc2.settings.dat` (desktop default server URL)
- `oc2.global.dat` e `oc2.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
3. Assicurati di avere l'ultima versione con `oc2 upgrade`
...
Per capire a quali modelli hai accesso, esegui `oc2 models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\oc2`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. oc2 installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\oc2`

2. Riavvia oc2 per reinstallare i pacchetti provider piu recenti

Questo forzera oc2 a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
oc2 rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
