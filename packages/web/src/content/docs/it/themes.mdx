...
Con OC2 puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OC2 usa il tema `oc2`.
...
OC2 include diversi temi integrati.
...
- Vuole che OC2 corrisponda all'aspetto del terminale
...
OC2 supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/oc2/themes/*.json` o `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.oc2/themes/*.json`
4. **Directory di lavoro corrente** - in `./.oc2/themes/*.json`
