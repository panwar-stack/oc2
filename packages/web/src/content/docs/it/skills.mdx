...
Le skill degli agenti permettono a OC2 di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OC2 cerca in queste posizioni:

- Config di progetto: `.oc2/skills/<name>/SKILL.md`
- Config globale: `~/.config/oc2/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OC2 risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.oc2/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/oc2/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.oc2/skills/git-release/SKILL.md` cosi':
...
OC2 elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `oc2.json`:
...
**Per agenti integrati** (in `oc2.json`):
