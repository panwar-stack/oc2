---
title: Server LSP
description: OC2 si integra con i tuoi server LSP.
---

OC2 può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

OC2 include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `OC2_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e oc2 apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di oc2.
