---
title: Plugin
description: Scrivi plugin per estendere OC2.
---

I plugin ti permettono di estendere OC2 agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OC2.
...
- `.oc2/plugins/` - plugin a livello progetto
- `~/.config/oc2/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/oc2/node_modules/`.
...
1. Config globale (`~/.config/oc2/oc2.json`)
2. Config di progetto (`oc2.json`)
3. Directory plugin globale (`~/.config/oc2/plugins/`)
4. Directory plugin di progetto (`.oc2/plugins/`)
...
OC2 esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di oc2 per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere oc2.
...
:::note
Se usi l'app desktop di OC2, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a oc2 di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a oc2:
...
L'helper `tool` crea uno strumento personalizzato che oc2 può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in oc2 insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://oc2.ai/docs/sdk) per i dettagli.
