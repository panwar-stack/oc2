...
Avec OC2, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OC2 utilise notre propre thème `oc2`.
...
OC2 est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OC2 corresponde à l'apparence de leur terminal
...
OC2 prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/oc2/themes/*.json` ou `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.oc2/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.oc2/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/oc2/themes
vim ~/.config/oc2/themes/my-theme.json
```
...
{
  "$schema": "https://oc2.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
