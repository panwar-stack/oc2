...
Vous pouvez ajouter des outils externes à OC2 à l'aide du _Model Context Protocol_ ou MCP. OC2 prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [OC2 Config](https://oc2.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/oc2`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
OC2 gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, OC2 :
...
Si le serveur nécessite une authentification, OC2 vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `oc2 mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, OC2 stockera les jetons en toute sécurité dans `~/.local/share/oc2/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans OC2, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration OC2 comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter OC2 à votre compte Sentry.
