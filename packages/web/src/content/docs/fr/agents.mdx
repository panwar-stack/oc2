...
Il existe deux types d'agents dans OC2 ; agents primaires et sous-agents.
...
OC2 est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OC2 est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

OC2 est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `oc2.json` :
...
Si aucune température n'est spécifiée, OC2 utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OC2 et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OC2 utilise le format `provider/model-id`. Par exemple, si vous utilisez [OC2 Zen](/docs/zen), vous utiliserez `oc2/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `oc2 models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/panwar-stack/oc2).
:::
