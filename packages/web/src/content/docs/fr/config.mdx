---
title: Configuration
description: Utilisation de la configuration JSON OC2.
---

Vous pouvez configurer OC2 à l'aide d'un fichier de configuration JSON.

---

## Format

OC2 prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/oc2`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/oc2/oc2.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OC2_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`oc2.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.oc2`** - agents, commandes, plugins
6. **Configuration en ligne** (`OC2_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.oc2` et `~/.config/oc2` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/oc2`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale OC2 dans `~/.config/oc2/oc2.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/oc2/tui.json`.
...
Ajoutez `oc2.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque OC2 démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `OC2_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OC2_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.oc2`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.oc2`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`oc2.ai/config.json`**](https://oc2.ai/config.json).

La configuration TUI utilise [**`oc2.ai/tui.json`**](https://oc2.ai/tui.json).
...
Utilisez `OC2_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `oc2.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `oc2 serve` et `oc2 web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur OC2.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `oc2.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration OC2 via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, OC2 essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/oc2/agents/` ou `.oc2/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, OC2 reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`oc2 run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/oc2/commands/` ou `.oc2/commands/`. [En savoir plus ici](/docs/commands).
...
OC2 téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, oc2 **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent OC2 avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.oc2/plugins/` ou `~/.config/oc2/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre OC2 à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
