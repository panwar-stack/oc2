---
title: Plugins
description: Écrivez vos propres plugins pour étendre OC2.
---

Les plugins vous permettent d'étendre OC2 en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OC2.
...
- `.oc2/plugins/` - Plugins au niveau du projet
- `~/.config/oc2/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/oc2/node_modules/`.
...
1. Configuration globale (`~/.config/oc2/oc2.json`)
2. Configuration du projet (`oc2.json`)
3. Répertoire global des plugins (`~/.config/oc2/plugins/`)
4. Répertoire des plugins du projet (`.oc2/plugins/`)
...
OC2 exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK oc2 pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre oc2.
...
:::note
Si vous utilisez l'application de bureau OC2, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez oc2 de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à oc2 :
...
L'assistant `tool` crée un outil personnalisé que oc2 peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour oc2 aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://oc2.ai/docs/sdk) pour plus de détails.
