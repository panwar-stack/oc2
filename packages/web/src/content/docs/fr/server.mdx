---
title: Serveur
description: Interagissez avec le serveur oc2 via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `oc2 serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client oc2 peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `oc2.local` |
...
Définissez `OC2_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `oc2`, ou définissez `OC2_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `oc2 serve` et à `oc2 web`.
...
Lorsque vous exécutez `oc2`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur oc2 pour interagir avec oc2 par programmation.
:::

Cette architecture permet à oc2 de prendre en charge plusieurs clients et vous permet d'interagir avec oc2 par programme.

Vous pouvez exécuter `oc2 serve` pour démarrer un serveur autonome. Si vous avez le
oc2 TUI en cours d'exécution, `oc2 serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OC2 [IDE](/docs/ide).
...
Le serveur oc2 expose les API suivantes.
