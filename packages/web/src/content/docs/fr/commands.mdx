...
Vous pouvez ajouter des commandes personnalisées via la configuration OC2 ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre OC2 [config](/docs/config) :
...
oc2 comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
