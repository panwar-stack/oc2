...
Les compétences d'agent permettent à OC2 de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
OC2 recherche ces emplacements :

- Configuration du projet : `.oc2/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/oc2/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, OC2 remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.oc2/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/oc2/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.oc2/skills/git-release/SKILL.md` comme ceci :
...
OC2 répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `oc2.json` :
...
**Pour les agents intégrés** (dans `oc2.json`) :
