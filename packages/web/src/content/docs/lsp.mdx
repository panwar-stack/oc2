---
title: LSP Servers
description: OC2 integrates with your LSP servers.
---

OC2 can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

OC2 comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OC2_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and oc2 opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your oc2 config.
