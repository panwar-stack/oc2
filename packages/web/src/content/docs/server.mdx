---
title: Server
description: Interact with oc2 server over HTTP.
---
...
The `oc2 serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an oc2 client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `oc2.local` |
...
Set `OC2_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `oc2`, or set `OC2_SERVER_USERNAME` to override it. This applies to both `oc2 serve` and `oc2 web`.
...
When you run `oc2` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the oc2 server to interact with oc2 programmatically.
:::

This architecture lets oc2 support multiple clients and allows you to interact with oc2 programmatically.

You can run `oc2 serve` to start a standalone server. If you have the
oc2 TUI running, `oc2 serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OC2 [IDE](/docs/ide) plugins.
...
The oc2 server exposes the following APIs.
...
| `POST`   | `/session/:id/init`                      | Run `/init` to create or update `AGENTS.md` | body: `{ messageID, providerID, modelID }`, returns `boolean`                      |
