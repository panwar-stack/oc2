---
title: Servidor
description: Interaja com o servidor oc2 via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `oc2 serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente oc2 pode usar.
...
```bash
oc2 serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `oc2.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
oc2 serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OC2_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `oc2`, ou defina `OC2_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `oc2 serve` quanto ao `oc2 web`.

```bash
OC2_SERVER_PASSWORD=your-password oc2 serve
```
...
Quando você executa `oc2`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor oc2 para interagir com o oc2 programaticamente.
:::

Essa arquitetura permite que o oc2 suporte múltiplos clientes e permite que você interaja com o oc2 programaticamente.

Você pode executar `oc2 serve` para iniciar um servidor autônomo. Se você tiver o TUI do oc2 em execução, `oc2 serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do oc2 [IDE](/docs/ide).
...
O servidor oc2 expõe as seguintes APIs.
