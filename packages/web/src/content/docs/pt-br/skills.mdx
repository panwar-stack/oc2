...
As habilidades do agente permitem que o oc2 descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O oc2 pesquisa nesses locais:

- Configuração do projeto: `.oc2/skills/<name>/SKILL.md`
- Configuração global: `~/.config/oc2/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o oc2 sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.oc2/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/oc2/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.oc2/skills/git-release/SKILL.md` assim:
...
O oc2 lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `oc2.json`:
...
**Para agentes embutidos** (em `oc2.json`):
