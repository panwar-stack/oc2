---
title: Plugins
description: Escreva seus próprios plugins para estender o oc2.
---

Plugins permitem que você estenda o oc2 conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do oc2.
...
- `.oc2/plugins/` - Plugins em nível de projeto
- `~/.config/oc2/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/oc2/node_modules/`.
...
1. Configuração global (`~/.config/oc2/oc2.json`)
2. Configuração do projeto (`oc2.json`)
3. Diretório de plugins global (`~/.config/oc2/plugins/`)
4. Diretório de plugins do projeto (`.oc2/plugins/`)
...
O oc2 executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do oc2 para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o oc2.
...
:::note
Se você estiver usando o aplicativo desktop oc2, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o oc2 de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao oc2:
...
O helper `tool` cria uma ferramenta personalizada que o oc2 pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o oc2 junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://oc2.ai/docs/sdk) para detalhes.
