...
Você pode adicionar ferramentas externas ao oc2 usando o _Model Context Protocol_, ou MCP. O oc2 suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do oc2](https://oc2.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/oc2`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O oc2 lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o oc2 irá:
...
Se o servidor requer autenticação, o oc2 solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `oc2 mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o oc2 armazenará os tokens de forma segura em `~/.local/share/oc2/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no oc2, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do oc2 como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o oc2 à sua conta do Sentry.
