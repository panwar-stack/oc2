...
Existem dois tipos de agentes no oc2; agentes primários e subagentes.
...
oc2 vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OC2 vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

OC2 vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do OC2, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `oc2.json`:
...
- Global: `~/.config/oc2/agents/`
- Por projeto: `.oc2/agents/`

```markdown title="~/.config/oc2/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o oc2 usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do oc2 quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do oc2 usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OC2 Zen](/docs/zen), você usaria `oc2/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `oc2 models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/panwar-stack/oc2).
:::
...
```markdown title="~/.config/oc2/agents/docs-writer.md"
---
...
```markdown title="~/.config/oc2/agents/security-auditor.md"
---
