...
Com o oc2, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o oc2 usa nosso próprio tema `oc2`.
...
O oc2 vem com vários temas embutidos.
...
- Querem que o oc2 corresponda à aparência do seu terminal
...
O oc2 suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/oc2/themes/*.json` ou `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.oc2/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.oc2/themes/*.json`
