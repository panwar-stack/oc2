...
As ferramentas permitem que o LLM execute ações em sua base de código. O oc2 vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no oc2.
...
:::note
Esta ferramenta está disponível apenas quando `OC2_EXPERIMENTAL_LSP_TOOL=true` (ou `OC2_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor oc2 ou quando a variável de ambiente `OC2_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o oc2:
