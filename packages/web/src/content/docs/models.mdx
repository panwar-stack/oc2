...
`logu/logu` can also be selected from the model picker when `local_fusion.logu` is configured. It is a local compound model that runs the reserved Logu workflow instead of calling a provider directly. Configure it first in [Local Fusion](/docs/local-fusion#logu-local-model), then select `logu/logu` from `/models`.
