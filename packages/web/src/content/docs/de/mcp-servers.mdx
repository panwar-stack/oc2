...
Sie können externe Tools zu OC2 hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. OC2 unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [OC2 Config](https://oc2.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/oc2`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
OC2 übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird OC2 Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von OC2 beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `oc2 mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert OC2 die Token sicher in `~/.local/share/oc2/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in OC2 verfügbar. Sie können sie also wie jedes andere Tool über die OC2-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und OC2 mit Ihrem Sentry-Konto zu verbinden.
