...
Agent Skills erlauben OC2, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Bei projektlokalen Pfaden laeuft OC2 vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.oc2/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/oc2/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.oc2/skills/git-release/SKILL.md` zum Beispiel so:
...
OC2 listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `oc2.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `oc2.json`):
