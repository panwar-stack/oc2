...
description: Erstellen Sie Tools, die der LLM in OC2 aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von OC2 wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.oc2/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/oc2/tools/` platzieren.
