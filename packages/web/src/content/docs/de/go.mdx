...
OC2 Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in OC2. Du abonnierst OC2 Go und
...
OC2 Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

OC2 Go funktioniert wie jeder andere Provider in OC2.

1. Du meldest dich bei **<a href={console}>OC2 Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OC2 Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OC2 Go abonnieren.
:::
...
OC2 Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner OC2 Config verwendet das Format `oc2-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `oc2-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben OC2 Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit OC2 zu nutzen.
