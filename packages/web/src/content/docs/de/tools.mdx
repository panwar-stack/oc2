...
OC2 bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in OC2 verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `OC2_EXPERIMENTAL_LSP_TOOL=true` (oder `OC2_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OC2-Provider genutzt wird oder die Umgebungsvariable `OC2_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von OC2:
