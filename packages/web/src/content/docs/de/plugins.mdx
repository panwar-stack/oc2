---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OC2 zu erweitern.
---

Mit Plugins können Sie OC2 erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OC2 zu ändern.
...
- `.oc2/plugins/` – Plugins auf Projektebene
- `~/.config/oc2/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/oc2/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/oc2/oc2.json`)
2. Projektkonfiguration (`oc2.json`)
3. Globales Plugin-Verzeichnis (`~/.config/oc2/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.oc2/plugins/`)
...
OC2 führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OC2-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OC2 erweitern können.
...
:::note
Wenn Sie die OC2-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OC2 `.env`-Dateien liest:
...
Plugins können OC2 auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OC2 aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OC2 zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://oc2.ai/docs/sdk).
