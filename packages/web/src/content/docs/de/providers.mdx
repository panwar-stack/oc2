---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OC2.
---

import config from "../../../../config.mjs"
export const console = config.console

OC2 verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OC2-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem OC2 anpassen
config.
...
OC2 Zen ist eine Liste von Modellen, die vom OC2-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OC2 funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OC2 Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OC2 aus und gehen Sie zu [oc2.ai/auth](https://oc2.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in OC2 und ist völlig optional.

---

## OC2 Go

OC2 Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OC2-Team getestet und verifiziert wurden, dass sie gut mit OC2 funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OC2 Go` und gehen Sie zu [oc2.ai/auth](https://oc2.ai/zen).
...
So verwenden Sie Amazon Bedrock mit OC2:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OC2 ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `oc2.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OC2 wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können oc2 so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OC2 ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OC2-Konfiguration hinzufügen.
...
um das Token im OC2-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OC2 verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OC2
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`oc2.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OC2:
...
So verwenden Sie Google Vertex AI mit OC2:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OC2 ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OC2 konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede OC2-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OC2-Konversation als separate Sitzung aufgeführt.
...
Sie können OC2 für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können OC2 über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können OC2 über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OC2 konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit OC2:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OC2.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OC2 verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OC2 Zen ist eine Liste getesteter und verifizierter Modelle, die vom OC2-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OC2 Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OC2 Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OC2 API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OC2-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OC2-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OC2:
...
Sie können Modelle auch über Ihre OC2-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OC2 verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `oc2.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann OC2 erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `oc2 auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OC2-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OC2-Konfiguration übereinstimmt.
