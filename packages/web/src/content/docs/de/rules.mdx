---
title: Regeln
description: Lege eigene Anweisungen fuer oc2 fest.
---

Du kannst oc2 ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in oc2 den Befehl `/init` aus.
...
So kann oc2 sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht oc2 sie zu erweitern.
...
oc2 kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/oc2/AGENTS.md` speichern.
Diese gelten in allen oc2-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OC2 dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/oc2/AGENTS.md` existiert)
...
Beim Start sucht oc2 in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/oc2/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `oc2.json` oder in `~/.config/oc2/oc2.json` angeben.
...
oc2 parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit oc2.json

Empfohlen ist das `instructions`-Feld in der `oc2.json`:
...
Du kannst oc2 explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `oc2.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
