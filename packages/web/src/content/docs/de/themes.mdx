...
Mit OC2 kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OC2 das `oc2`-Theme.
...
OC2 liefert mehrere eingebaute Themes mit.
...
- OC2 optisch ans Terminal anpassen wollen
...
OC2 unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/oc2/themes/*.json` oder `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.oc2/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.oc2/themes/*.json`
