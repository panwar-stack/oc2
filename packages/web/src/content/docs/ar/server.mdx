---
title: الخادم
description: تفاعل مع خادم oc2 عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `oc2 serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل oc2 استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `oc2.local` |
...
عيّن `OC2_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `oc2`، أو عيّن `OC2_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `oc2 serve` و `oc2 web`.
...
عند تشغيل `oc2` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم oc2 للتفاعل مع oc2 برمجيا.
:::

تتيح هذه البنية لـ oc2 دعم عدة عملاء وتمكّنك من التفاعل مع oc2 برمجيا.

يمكنك تشغيل `oc2 serve` لبدء خادم مستقل. إذا كانت واجهة oc2 في terminal (TUI)
قيد التشغيل، فسيبدأ `oc2 serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OC2 لـ [IDE](/docs/ide).
...
يعرض خادم oc2 واجهات API التالية.
