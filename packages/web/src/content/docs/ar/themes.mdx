...
مع OC2 يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OC2 سمتنا `oc2`.
...
- يريدون أن يطابق OC2 مظهر terminal لديهم
...
يدعم OC2 نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/oc2/themes/*.json` أو `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.oc2/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.oc2/themes/*.json`
