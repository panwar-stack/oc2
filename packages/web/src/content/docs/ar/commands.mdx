...
أنشئ الملف `.oc2/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OC2 أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OC2:
...
- على مستوى النظام: `~/.config/oc2/commands/`
- لكل مشروع: `.oc2/commands/`
...
يتضمن oc2 عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
