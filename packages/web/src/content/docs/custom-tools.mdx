---
title: Custom Tools
description: Create tools the LLM can call in oc2.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside oc2's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.oc2/tools/` directory of your project.
- Or globally, by placing them in `~/.config/oc2/tools/`.
