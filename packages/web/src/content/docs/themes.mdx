...
With OC2 you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OC2 uses our own `oc2` theme.
...
OC2 comes with several built-in themes.
...
- Want OC2 to match their terminal's appearance
...
OC2 supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/oc2/themes/*.json` or `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.oc2/themes/*.json`
4. **Current working directory** - Defined in `./.oc2/themes/*.json`
