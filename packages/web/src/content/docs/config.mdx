...
Branches and the judge are research stages. If Logu grants write-capable policies to a branch or judge, writes and edits are isolated to a temporary scratch directory outside session roots, and `apply_patch` remains unavailable. Scratch retention is unspecified; only the synthesizer should apply final edits to workspace files when its tools and permissions allow it.
