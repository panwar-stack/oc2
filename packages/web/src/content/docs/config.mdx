...
- `branches` - Candidate models that receive the original request context. At least one branch is required when `fugu/fugu` is used.
- `judge` - Optional model config that is parsed and preserved. It is not executed in the first runtime pass.
- `synthesizer` - Final model config used to produce the caller-visible response. It is required when `fugu/fugu` is used.

Branch, judge, and synthesizer targets must resolve to normal models and cannot point back to `fugu/fugu`; circular routing is rejected. In the first pass, hidden branch and synthesizer calls do not execute tools.
