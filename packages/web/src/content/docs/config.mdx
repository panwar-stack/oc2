...
Generated schemas and SDK types permit omitted target models so OpenCode can start, but each configured branch, judge, and synthesizer target still needs a `model` when `fugu/fugu` is used.
