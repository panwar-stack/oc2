...
_Model Bağlam Protokolü_ veya MCP kullanarak oc2'a harici araçlar ekleyebilirsiniz. oc2 hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [oc2 Config](https://oc2.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/oc2` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
oc2, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde oc2 şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa oc2, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `oc2 mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından oc2, belirteçleri `~/.local/share/oc2/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra oc2'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi oc2 yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve oc2'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
