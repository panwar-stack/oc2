---
title: Sunucu
description: oc2 sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`oc2 serve` komutu, oc2 istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `oc2.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OC2_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `oc2` değeridir; değiştirmek isterseniz `OC2_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `oc2 serve` hem de `oc2 web` için geçerlidir.
...
`oc2` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
oc2 ile programatik etkileşim için oc2 sunucusunu kullanın.
:::

Bu mimari, oc2'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`oc2 serve` ile bağımsız bir sunucu başlatabilirsiniz. oc2 TUI açıksa, `oc2 serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum oc2 [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
oc2 sunucusu aşağıdaki API'leri sunar.
