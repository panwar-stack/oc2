...
OC2 Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OC2'daki diğer sağlayıcılar gibi çalışır. OC2 Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OC2'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OC2 Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OC2 Go, OC2'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OC2 Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OC2 Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OC2 Go'ya abone olabilir.
:::
...
OC2 Go aşağıdaki limitleri içerir:
...
OC2 yapılandırmanızdaki [model id](/docs/config/#models) formatı `oc2-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `oc2-go/kimi-k2.6` kullanmalısınız.
...
OC2 Go'yu şu amaçlarla oluşturduk:
...
4. OC2 ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
