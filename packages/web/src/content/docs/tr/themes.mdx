...
oc2 ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak oc2 kendi `oc2` temasını kullanır.
...
oc2 birden fazla yerleşik temayla gelir.
...
- oc2'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
oc2, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/oc2/themes/*.json` veya `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.oc2/themes/*.json`
4. **Geçerli çalışma dizini** - `./.oc2/themes/*.json`
