...
description: OC2, LSP sunucularınızla bütünleşir.
---

OC2, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

OC2, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OC2_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve oc2 bir dosyayı açtığında:
...
LSP sunucularını, oc2 yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
