...
Run local compound model orchestration from inside the current session. The tool fans one prompt out to multiple inline branch models, judges the branch outputs, and synthesizes one final answer.
...
Use `local_fusion` when you want a local research panel or model comparison without routing through a remote compound-model provider. The first version is explicit: the tool input must include `branches`, `judge`, and `synthesizer` inline.

:::note
Named compound configs are not available yet. Passing `config` to `local_fusion` returns an error; pass inline `branches`, `judge`, and `synthesizer` instead.
:::
...
:::caution
`local_fusion` does not provide `model: "compound/..."` routing, branch trace UI, crash recovery records, or mutating branch tools in this first version.
:::
