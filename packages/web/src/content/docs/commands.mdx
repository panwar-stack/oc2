...
Custom commands are in addition to the built-in commands like `/init`, `/clarify`, `/local_fusion`, `/spec-planner`, `/undo`, `/redo`, `/share`, `/help`. [Learn more](/docs/tui#commands).

Use `/local_fusion` with a named [`local_fusion` config](/docs/config#local-fusion):
...
Create `.oc2/commands/test.md`:

```md title=".oc2/commands/test.md"
---
...
You can add custom commands through the OC2 config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OC2 [config](/docs/config):

```json title="oc2.jsonc" {4-12}
{
...
- Global: `~/.config/oc2/commands/`
- Per-project: `.oc2/commands/`

```markdown title="~/.config/oc2/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".oc2/commands/component.md"
---
...
```md title=".oc2/commands/create-file.md"
---
...
```md title=".oc2/commands/analyze-coverage.md"
---
...
```md title=".oc2/commands/review-changes.md"
---
...
```md title=".oc2/commands/review-component.md"
---
...
oc2 includes several built-in commands like `/init`, `/clarify`, `/spec-planner`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
