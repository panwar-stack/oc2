...
U OC2 mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OC2 koristi nasu `oc2` temu.
...
OC2 dolazi sa vise ugradenih tema.
...
- Zele da OC2 odgovara izgledu njihovog terminala
...
OC2 podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/oc2/themes/*.json` ili `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.oc2/themes/*.json`
4. **Trenutni radni direktorij** - `./.oc2/themes/*.json`
