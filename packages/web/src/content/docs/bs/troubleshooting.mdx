...
Da biste otklonili probleme s OC2, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\oc2\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `oc2 --log-level DEBUG`.

---

## Pohrana

OC2 pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/oc2/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\oc2`
...
OC2 Desktop pokreće lokalni OC2 server (`oc2-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OC2` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/oc2/oc2.jsonc` (ili `~/.config/oc2/oc2.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/oc2/oc2.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\oc2\oc2.jsonc`
...
OC2 također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/oc2/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\oc2\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OC2 mogao ponovo izgraditi.

1. Potpuno zatvorite OC2 Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/oc2`
- **Linux**: obrišite `~/.cache/oc2` (ili pokrenite `rm -rf ~/.cache/oc2`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\oc2`

3. Ponovo pokrenite OC2 Desktop.

---

### Rješavanje problema sa vezom na serveru

OC2 Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `oc2.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OC2_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OC2_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OC2 Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OC2 karakteristikama.

---

### Obavještenja se ne prikazuju

OC2 Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OC2 u postavkama vašeg OS-a, i
...
1. Zatvorite OC2 Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OC2 Desktop aplikacije):

- `oc2.settings.dat` (URL zadanog servera za desktop)
- `oc2.global.dat` i `oc2.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `oc2 upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `oc2 models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\oc2`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OC2 dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\oc2`

2. Ponovo pokrenite OC2 da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OC2 da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OC2 će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
