...
OC2 Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u OC2-u. Pretplatite se na OC2 Go i
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa OC2-om.
:::
...
OC2 Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

OC2 Go radi kao bilo koji drugi provajder u OC2-u.

1. Prijavite se na **<a href={console}>OC2 Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `OC2 Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na OC2 Go.
:::
...
OC2 Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj OC2 konfiguraciji
koristi format `oc2-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`oc2-go/kimi-k2.6` u svojoj konfiguraciji.
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz OC2 koristite i bilo kojeg drugog provajdera.
