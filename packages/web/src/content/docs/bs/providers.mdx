---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OC2.
---

import config from "../../../../config.mjs"
export const console = config.console

OC2 koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OC2 konfiguraciji.
...
u `~/.local/share/oc2/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OC2
config.
...
OC2 Zen je lista modela koje je obezbedio OC2 tim koji su bili
testirano i potvrđeno da dobro radi sa OC2. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OC2 Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite oc2 i idite na [oc2.ai/auth](https://oc2.ai/auth).
...
Radi kao i svaki drugi provajder u OC2 i potpuno je opcionalan za korištenje.

---

## OC2 Go

OC2 Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OC2 tim i koji su testirani i verificirani da dobro rade s OC2-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OC2 Go` i idite na [oc2.ai/auth](https://oc2.ai/zen).
...
Da biste koristili Amazon Bedrock s OC2:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete oc2:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `oc2.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OC2.
:::
...
Možete konfigurirati oc2 za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi oc2 ispravno radio.
   :::
...
Također možete dodati modele kroz svoju oc2 konfiguraciju.
...
da pohrani token u oc2 auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OC2 koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OC2
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`oc2.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s oc2:
...
Za korištenje Google Vertex AI s OC2:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće oc2.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki OC2 razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OC2 razgovor naveden kao zasebna sesija.
...
Možete konfigurirati oc2 za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati oc2 za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati oc2 za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OC2. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s OC2:
...
3. Kopirajte API ključ za korištenje u OC2.
...
6. **Važno**: Prije upotrebe modela oblaka u OC2, morate lokalno povući informacije o modelu:
...
OC2 Zen je lista testiranih i verifikovanih modela koju je obezbedio OC2 tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OC2 Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OC2 Zen**.

```txt
   /connect
```

3. Unesite svoj OC2 API ključ.
...
Također možete dodati dodatne modele putem vaše oc2 konfiguracije.
...
5. Također ih možete prilagoditi putem vaše oc2 konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju oc2 konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s oc2-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `oc2.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OC2 da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `oc2 auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OC2 konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj oc2 konfiguraciji.
