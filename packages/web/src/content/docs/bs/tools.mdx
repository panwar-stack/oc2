...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OC2 dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OC2.
...
:::note
Ovaj alat je dostupan samo kada je `OC2_EXPERIMENTAL_LSP_TOOL=true` (ili `OC2_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OC2 provajdera ili kada je varijabla `OC2_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OC2:
