---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OC2.
---

Dodaci vam omogućavaju da proširite OC2 spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OC2.
...
- `.oc2/plugins/` - Dodaci na nivou projekta
- `~/.config/oc2/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/oc2/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/oc2/oc2.json`)
2. Konfiguracija projekta (`oc2.json`)
3. Globalni direktorij dodataka (`~/.config/oc2/plugins/`)
4. Direktorij dodataka projekta (`.oc2/plugins/`)
...
OC2 pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OC2.
...
Spriječite oc2 da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u oc2:
...
Pomoćnik `tool` kreira prilagođeni alat koji oc2 može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za oc2 zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://oc2.ai/docs/sdk) za detalje.
