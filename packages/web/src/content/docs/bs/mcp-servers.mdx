...
Možete dodati vanjske alate u OC2 koristeći _Model Context Protocol_, ili MCP. OC2 podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OC2 Config](https://oc2.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/oc2`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OC2 automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OC2 će:
...
Ako server zahtijeva autentifikaciju, OC2 će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `oc2 mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OC2 sigurno čuva tokene u `~/.local/share/oc2/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OC2, zajedno s ugrađenim alatima. Možete njima upravljati kroz OC2 konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OC2 sa vašim Sentry nalogom.
