...
Postoje dvije vrste agenata u OC2: primarni agenti i podagenti.
...
OC2 dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OC2 dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OC2 dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u OC2-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `oc2.json`:
...
- Globalno: `~/.config/oc2/agents/`
- Po projektu: `.oc2/agents/`
...
Ako temperatura nije navedena, OC2 koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OC2 konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OC2 konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OC2 Zen](/docs/zen), koristili biste `oc2/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `oc2 models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/panwar-stack/oc2).
:::
