---
title: Konfiguracija
description: Korištenje OC2 JSON konfiguracije.
---

Možete konfigurirati OC2 koristeći JSON konfiguracijski fajl.

---

## Format

OC2 podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/oc2`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/oc2/oc2.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OC2_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`oc2.json` u projektu) - postavke specifične za projekat
5. **`.oc2` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OC2_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.oc2` i `~/.config/oc2` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/oc2` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OC2 konfiguraciju u `~/.config/oc2/oc2.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/oc2/tui.json`.
...
Dodajte `oc2.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OC2 pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OC2_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OC2_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.oc2` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.oc2`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`oc2.ai/config.json`**](https://oc2.ai/config.json).

TUI konfiguracija koristi [**`oc2.ai/tui.json`**](https://oc2.ai/tui.json).
...
Koristite `OC2_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `oc2.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `oc2 serve` i `oc2 web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OC2 server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `oc2.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OC2 konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OC2 pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/oc2/agents/` ili `.oc2/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OC2 će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`oc2 run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/oc2/commands/` ili `.oc2/commands/`. [Saznajte više ovdje](/docs/commands).
...
OC2 će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OC2 **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OC2 sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.oc2/plugins/` ili `~/.config/oc2/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OC2 da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
