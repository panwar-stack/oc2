---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OC2.
---

import config from "../../../../config.mjs"
export const console = config.console

OC2 bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OC2-konfigurasjonen.
...
i `~/.local/share/oc2/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OC2
konfig.
...
OC2 Zen er en liste over modeller levert av OC2-teamet som har vært
testet og verifisert for å fungere godt med OC2. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OC2 Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OC2 Zen` og gå til [oc2.ai/auth](https://oc2.ai/zen).
...
Det fungerer som alle andre leverandører i OC2 og er helt valgfritt å bruke.

---

## OC2 Go

OC2 Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OC2-teamet som har vært
testet og verifisert for å fungere godt med OC2.

1. Kjør kommandoen `/connect` i TUI, velg `OC2 Go`, og gå til [oc2.ai/auth](https://oc2.ai/zen).
...
Slik bruker du Amazon Bedrock med OC2:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OC2:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `oc2.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i oc2 støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere oc2 til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OC2 skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OC2-konfigurasjonen.
...
for å lagre token i OC2 auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OC2 bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OC2
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`oc2.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med OC2:
...
Slik bruker du Google Vertex AI med OC2:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OC2, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver OC2-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OC2-samtale oppført som en separat økt.
...
Du kan konfigurere OC2 for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OC2 for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OC2 for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OC2. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OC2:
...
3. Kopier API-nøkkelen for bruk i OC2.
...
6. **Viktig**: Før du bruker skymodeller i OC2, må du hente modellinformasjonen lokalt:
...
OC2 Zen er en liste over testede og verifiserte modeller levert av OC2-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OC2 Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OC2 Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OC2 API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OC2-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OC2-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OC2:
...
Du kan også tilpasse modeller gjennom OC2-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OC2. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `oc2.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OC2 forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `oc2 auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OC2-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OC2-konfigurasjonen.
