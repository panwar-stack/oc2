---
title: Tjener
description: Samhandle med OC2-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `oc2 serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OC2-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `oc2.local` |
...
Sett `OC2_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `oc2` som standard, eller sett `OC2_SERVER_USERNAME` for å overstyre det. Dette gjelder både `oc2 serve` og `oc2 web`.
...
Når du kjører `oc2` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OC2-serveren til å samhandle med OC2 programmatisk.
:::

Denne arkitekturen lar OC2 støtte flere klienter og lar deg samhandle med OC2 programmatisk.

Du kan kjøre `oc2 serve` for å starte en frittstående server. Hvis du har
OC2 TUI kjører, vil `oc2 serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OC2 [IDE](/docs/ide) plugins.
...
oc2-serveren viser følgende APIer.
