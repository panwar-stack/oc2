---
title: Konfigurasjon
description: Ved å bruke oc2 JSON-konfigurasjonen.
---

Du kan konfigurere OC2 ved å bruke en JSON konfigurasjonsfil.

---

## Format

OC2 støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "oc2"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/oc2`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/oc2/oc2.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OC2_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`oc2.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.oc2` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`OC2_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.oc2` og `~/.config/oc2` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/oc2`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OC2-konfigurasjon i `~/.config/oc2/oc2.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `oc2.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når OC2 starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `OC2_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `OC2_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.oc2` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.oc2`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`oc2.ai/config.json`**](https://oc2.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `oc2 serve` og `oc2 web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage OC2-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `oc2.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i OC2-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver OC2 å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/oc2/agents/` eller `.oc2/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil OC2 falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`oc2 run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/oc2/commands/` eller `.oc2/commands/`. [Les mer her](/docs/commands).
...
OC2 vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater OC2 alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider OC2 med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.oc2/plugins/` eller `~/.config/oc2/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense OC2 til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
