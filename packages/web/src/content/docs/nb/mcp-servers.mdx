...
Du kan legge til eksterne verktøy i OC2 ved å bruke _Model Context Protocol_ eller MCP. OC2 støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OC2 Config](https://oc2.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="oc2.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/oc2`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
OC2 håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OC2:
...
Hvis serveren krever autentisering, vil OC2 be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `oc2 mcp auth <server-name>`.
...
```bash
oc2 mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OC2 lagre tokens sikkert i `~/.local/share/oc2/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i OC2, sammen med innebygde verktøy. Så du kan administrere dem gjennom OC2-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OC2 til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OC2-samtale oppført som en separat økt.
