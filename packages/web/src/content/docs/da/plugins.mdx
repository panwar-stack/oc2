---
title: Plugins
description: Skriv dine egne plugins for at udvide OC2.
---

Plugins giver dig mulighed for at udvide OC2 ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OC2s standardadfærd.
...
- `.oc2/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/oc2/node_modules/`.
...
1. Global konfiguration (`~/.config/oc2/oc2.json`)
2. Projektkonfiguration (`oc2.json`)
3. Global plugin-mappe (`~/.config/oc2/plugins/`)
4. Projekt plugin bibliotek (`.oc2/plugins/`)
...
OC2 kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En oc2 SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide oc2.
...
:::note
Hvis du bruger OC2 desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Plugins kan også tilføje brugerdefinerede værktøjer til oc2:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som oc2 kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for oc2 sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://oc2.ai/docs/sdk) for detaljer.
