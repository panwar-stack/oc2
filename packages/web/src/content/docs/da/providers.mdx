---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OC2.
---

import config from "../../../../config.mjs"
export const console = config.console

OC2 bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OC2-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/oc2/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OC2-konfigurationen.
...
OC2 Zen er en liste over modeller leveret af OC2-teamet, som er blevet testet og verificeret til at fungere godt med OC2. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OC2 Zen.
:::
...
Det fungerer som alle andre udbydere i OC2 og er helt valgfrit at bruge.

---

## OC2 Go

OC2 Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OC2-teamet, som er testet og verificeret til at fungere godt med OC2.

1. Kør kommandoen `/connect` i TUI, vælg `OC2 Go`, og gå til [oc2.ai/auth](https://oc2.ai/zen).
...
Sådan bruger du Amazon Bedrock med OC2:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører oc2:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `oc2.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OC2 understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere oc2 til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at oc2 skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem oc2-konfigurationen.
...
gemme tokenet i oc2 auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OC2 bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OC2
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`oc2.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med OC2:
...
Sådan bruger du Google Vertex AI med OC2:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem oc2, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver OC2-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OC2-samtale opført som en separat session.
...
Du kan konfigurere oc2 til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere oc2 til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere oc2 til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OC2. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OC2:
...
3. Kopier API-nøglen til brug i OC2.
...
6. **Vigtigt**: Før du bruger skymodeller i OC2, skal du hente modelinformationen lokalt:
...
OC2 Zen er en liste over testede og verificerede modeller leveret af OC2-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OC2 Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OC2 Zen**.

   ```txt
   /connect
   ```

3. Indtast OC2 API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem oc2-konfigurationen.
...
5. Du kan også tilpasse dem gennem oc2-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OC2:
...
Du kan også tilpasse modeller gennem oc2-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med oc2. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `oc2.json`-filen i projektkataloget:
...
`limit`-felterne lader OC2 forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `oc2 auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek oc2-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i oc2-konfigurationen.
