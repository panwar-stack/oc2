---
title: Konfiguration
description: Ved at bruge OC2 JSON konfig.
---

Du kan konfigurere OC2 ved hjælp af en JSON-konfigurationsfil.

---

## Format

OC2 understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/oc2`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/oc2/oc2.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OC2_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`oc2.json` i projekt) - projektspecifikke indstillinger
5. **`.oc2` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OC2_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.oc2` og `~/.config/oc2` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/oc2`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OC2-konfiguration i `~/.config/oc2/oc2.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `oc2.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når OC2 starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OC2_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OC2_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.oc2` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.oc2` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`oc2.ai/config.json`**](https://oc2.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `oc2 serve` og `oc2 web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din OC2-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `oc2.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din OC2-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger OC2 at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din OC2-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/oc2/agents/` eller `.oc2/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil OC2 falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`oc2 run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/oc2/commands/` eller `.oc2/commands/`. [Learn more here](/docs/commands).
...
OC2 vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard oc2 **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide OC2 med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.oc2/plugins/` eller `~/.config/oc2/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse OC2 til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
