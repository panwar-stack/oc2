...
Med OC2 kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OC2 vårt eget `oc2`-tema.
...
OC2 kommer med flere innebygde temaer.
...
- Vil at OC2 skal matche terminalens utseende
...
OC2 støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/oc2/themes/*.json` eller `$XDG_CONFIG_HOME/oc2/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.oc2/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.oc2/themes/*.json`
