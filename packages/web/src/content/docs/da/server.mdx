---
title: Server
description: Interager med oc2-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `oc2 serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en oc2-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `oc2.local` |
...
Indstil `OC2_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `oc2`, eller indstil `OC2_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `oc2 serve` og `oc2 web`.
...
Når du kører `oc2` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug oc2-serveren til at interagere med oc2 programmatisk.
:::

Denne arkitektur lader oc2 understøtte flere klienter og lader dig interagere med oc2 programmatisk.

Du kan køre `oc2 serve` for at starte en selvstændig server. Hvis du har
oc2 TUI kørende, vil `oc2 serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OC2 [IDE](/docs/ide) plugins.
...
OC2-serveren viser følgende API'er.
