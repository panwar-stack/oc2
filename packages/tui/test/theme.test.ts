...
import { discoverThemes, getStartupThemeMode } from "../src/context/theme"
...
test.each(["dark", "light"] as const)("startup uses available %s terminal mode", (mode) => {
  expect(getStartupThemeMode({ themeMode: mode })).toBe(mode)
})

test("startup falls back to dark while terminal theme detection is pending", () => {
  expect(getStartupThemeMode({ themeMode: null })).toBe("dark")
})
