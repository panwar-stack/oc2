import type { TuiPluginApi } from "@oc2-ai/plugin/tui"
...
const TIPS: Tip[] = [
  ...
  (shortcuts) => press(shortcuts.inputPaste(), "to paste from your clipboard; OC2 shows feedback while it reads"),
  ...
  "Create {highlight}oc2.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/oc2/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Run {highlight}/use-team{/highlight} to split complex work across teammate agents",
  "Select {highlight}fugu/fugu{/highlight} from {highlight}/models{/highlight} to use the Fugu model",
  "Run {highlight}/local:fusion{/highlight} after setting {highlight}local_fusion{/highlight} config for local fusion",
  "Run {highlight}/spec:planner{/highlight} to draft a spec, then {highlight}/spec:implement{/highlight} to build it",
  "Use {highlight}/mcps{/highlight} to inspect and manage configured MCP servers",
  "Use {highlight}/skills{/highlight} to browse available skills and insert the right one into your prompt",
  "Use {highlight}prompt stash{/highlight} from the command palette to save a draft and pop it back later",
  "Open the {highlight}plugin manager{/highlight} from the command palette to inspect and toggle TUI plugins",
  "Use {highlight}which-key{/highlight} to discover active keybindings and leader-key sequences without leaving the TUI",
  "Watch {highlight}prompt-cache{/highlight} status in usage details to see reads, writes, and token savings",
  "Add {highlight}.md{/highlight} files to {highlight}.oc2/commands/{/highlight} to define reusable custom prompts",
  "Add {highlight}.md{/highlight} files to {highlight}.oc2/agents/{/highlight} for specialized AI personas",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Use {highlight}oc2 run{/highlight} for non-interactive scripting",
  "Use {highlight}oc2 --continue{/highlight} to resume the last session",
  "Use {highlight}oc2 run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}oc2 serve{/highlight} for headless API access to OC2",
  "Use {highlight}oc2 run --attach{/highlight} to connect to a running server",
  "Run {highlight}oc2 upgrade{/highlight} to update to the latest version",
  "Run {highlight}oc2 auth list{/highlight} to see all configured providers",
  "Run {highlight}oc2 agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.oc2/themes/{/highlight} directory",
  ...
  "Run {highlight}oc2 debug config{/highlight} to troubleshoot configuration",
