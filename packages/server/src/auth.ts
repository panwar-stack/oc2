...
export function header(credentials?: Credentials) {
  const password = credentials?.password ?? Naming.env("OC2_SERVER_PASSWORD")
  if (!password) return undefined

  return `Basic ${Buffer.from(`${credentials?.username ?? Naming.env("OC2_SERVER_USERNAME") ?? DEFAULT_USERNAME}:${password}`).toString("base64")}`
}
