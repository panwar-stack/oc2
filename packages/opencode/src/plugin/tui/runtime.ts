...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@oc2-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import * as Log from "@oc2-ai/core/util/log"
import { errorData, errorMessage } from "@oc2-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@oc2-ai/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@oc2-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@oc2-ai/core/v1/config/plugin"
import { createCommandShim } from "@oc2-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@oc2-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".oc2"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".oc2", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".oc2", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OC2_PURE ? [] : pluginOrigins
    if (Flag.OC2_PURE && pluginOrigins.length) {
