...
<html>
  <head>
    <title>OC2 - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to OC2.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>OC2 - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "co2"
      output.headers["User-Agent"] = `oc2/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
