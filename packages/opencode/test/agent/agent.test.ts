...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(Permission.evaluate("edit", "packages/app/src/index.ts", task?.permission ?? []).action).toBe("allow")
      expect(Permission.evaluate("edit", "src/index.ts", planner?.permission ?? []).action).toBe("deny")
...
it.instance("plan agent denies edits except .oc2/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".oc2/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".oc2", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OC2_TEST_HOME
      process.env.OC2_TEST_HOME = test.directory
