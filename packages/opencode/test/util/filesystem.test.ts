...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "oc2.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "oc2.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "oc2.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "oc2.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["oc2.json", "oc2.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "oc2.json"),
        path.join(project, "oc2.jsonc"),
      ])
    })
  })
