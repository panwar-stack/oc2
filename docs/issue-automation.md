...
| Administration | Read  | Inspect repository rulesets and parent bypass capability    |

The publication token is created for the current repository only and has only Contents write and Pull requests write. Auto-merge uses two more repository-scoped tokens: a settings token with Administration read, Contents read, and Pull requests read, and a distinct mutation token with Contents read and Pull requests write. GitHub returns complete bypass actors for repository-owned rulesets, but hides inherited organization or enterprise actors from a repository-scoped token. For the inherited trusted-workflow rule, the helper instead requires `current_user_can_bypass: never`, directly proving that this App cannot bypass it. The trusted helper uses the settings token only for reads and never gives it to `gh`, Git, publication, or any mutation function. If Administration read does not reveal the required settings or ruleset details, automation remains disabled and fails closed; never escalate the inspection token to write. The App private key is provided only to the pinned token actions. Installation tokens are not put in remote URLs, artifacts, comments, generated files, or logs, and action post-processing revokes them.
...
Verification runs the fixed checks in a digest-pinned container with no network, all capabilities dropped, no new privileges, a read-only root filesystem, bounded CPU, memory, processes, files, output, and time. The entrypoint first proves the candidate source bind is read-only, then copies repository files into a bounded writable `/workspace` tmpfs. Every fixed check runs from that disposable copy so cache-cold Turbo builds can create and restore `dist` and other declared outputs. The source bind, Git metadata, and dependencies remain read-only, fixed caches stay bounded tmpfs, and container-declared environment variables or volumes are rejected. Container removal destroys the writable copy, and candidate bytes are rechecked after the sandbox finishes.

Generation can change ordinary files inside the supplied checkout, including runtime packages, tests, docs, specs, and non-TCB scripts. It cannot change workflows, automation agent descriptors, repository and dependency configuration, lockfiles, environment or Git control files, this operations guide or specification, the issue/verification/automation/publication/CI-scope scripts, or fixed generated/package/docs check implementations. The immutable agent edit policy and shared durable patch policy enforce the same protected-path boundary.
...
The helper re-fetches the PR and requires the same head plus exactly one of a REBASE auto-merge request or matching merge-queue entry. A `main` advance after the exact PR is published is allowed: the helper continues to bind the recorded base for source and Git provenance, while binding the live PR to the exact verified head, same repository, and base ref `main`; the merge queue applies its current-main and required-check rules. It repeats the complete repository/ruleset validation after enrollment before reporting success. It never calls update-branch, pushes or rebases `main`, directly merges a PR, or falls back to another merge method.

## Artifacts, Logs, And Retention

Admission, trusted helper, issue bundle, generation, and verification artifacts are retained for one day. Jobs delete downloaded and temporary state in `always()` cleanup steps. Terminal status does not require any artifact or successful admission output: it checks out the trusted workflow source and locates only the unique canonical bot-owned marker for the current event issue, run ID, and attempt before performing the read-update-read compare-and-swap. If that run never reserved a marker, status is a no-op. The durable provenance check intentionally does not depend on expiring artifacts; it uses the source verify job, immutable App-owned branch, current PR metadata, and exact Git diff.
...
| `stale_base`             | `main` moved before the exact automation branch and PR were published                              |
