...
- **TUI slash command:** Enter a slash action exposed by the current UI, such as `/sessions`, `/new`, `/models`, `/agents`, `/roots`, `/status`, `/themes`, `/help`, `/pause`, `/unpause`, or `/exit`. These operate the interface rather than sending a model prompt.
...
`/unpause` releases the pause owned by the viewed session and resumes the sessions that have durable resume intent and no remaining blockers. It is disabled while the viewed session does not own a pause. A child `/unpause` cannot clear an ancestor pause: if the session stays paused afterwards, the TUI reports that it remains blocked by an ancestor. The former `/start` name still works as a deprecated alias for `/unpause`.

Pause state is hydrated from the most recently updated sessions and self-corrects on the first `/pause` or `/unpause` action, which are idempotent.

While paused, prompts you send are queued rather than rejected and run after `/unpause`; side-effecting shell, command, and summarize actions return a paused error instead of partially executing. Shutdown and explicit cancellation remain available while paused. Pausing the lead of a team pauses every teammate; pausing a teammate pauses only that member's subtree.
