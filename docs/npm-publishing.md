...
be short-lived, grant read/write package access with `Bypass 2FA` enabled, be
limited to the bootstrap publication, and be revoked immediately afterward.
Never retain it as a repository secret or an `NPM_TOKEN` fallback in the normal
workflow. `script/bootstrap-npm` reads this granular token without echoing it,
stores it only in a mode-0600 temporary npmrc, and logs it out and removes it
after post-publication verification. A granular bootstrap token can publish the
initial packages, but it cannot configure trusted publishing.
...
authentication, not the granular bootstrap token. The bootstrap helper starts
this web login only after logging out the granular token:
