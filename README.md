...
- `oc2 run <prompt> [--json] [--model <provider/model>] [--root <path>]` runs a one-shot prompt through the main agent.
...
Use `--root <path>` with `run` or `tui` to add an allowed workspace root for file tools. Export transcripts as Markdown or JSON:
