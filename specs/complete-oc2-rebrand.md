...
- `package.json` is already named `oc2` and points at `https://github.com/panwar-stack/oc2`, but `scripts.sso` still uses `--sso-session=opencode` and `scripts.random` is nonessential.
...
- Workflow-driven scripts such as `script/github/close-issues.ts`, `script/github/close-prs.ts`, `script/duplicate-pr.ts`, `script/beta.ts`, and `script/stats.ts` remain while the workflows that call them remain.
- Likely stale one-off scripts include `script/setup-opencode-alias.sh`, `script/sign-windows.ps1`, `packages/enterprise/script/scrap.ts`, `packages/opencode/script/bench-search.ts`, `packages/http-recorder/script/pack.ts`, and `packages/llm/script/recording-cost-report.ts`.
...
- Delete zero-reference one-offs after confirmation: `script/setup-opencode-alias.sh`, `script/sign-windows.ps1`, `packages/enterprise/script/scrap.ts`, `packages/opencode/script/bench-search.ts`, `packages/http-recorder/script/pack.ts`, and `packages/llm/script/recording-cost-report.ts`.
- Delete workflow-only scripts such as `script/github/close-issues.ts`, `script/github/close-prs.ts`, `script/duplicate-pr.ts`, `script/beta.ts`, and `script/stats.ts` only after PR 6 removes the workflows that invoke them.
- Remove or rename root package aliases that are stale, misleading, or legacy-branded, including `random`, `dev:stats` if stats is removed, and `sso` if it still uses `opencode`.
