...
- Do not replace private `packages/opencode` imports with `@oc2-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@oc2-ai/tui`.
...
Status: Completed. The private `@oc2-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@oc2-ai/tui`.
...
primitives now live in `@oc2-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@oc2-ai/tui/runtime`. Movable app, component,
...
`@oc2-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@oc2-ai/tui`. Configured references resolve through the new
...
`@oc2-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@oc2-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@oc2-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@oc2-ai/core`, or either
  executable package.
...
rg '@oc2-ai/core|packages/opencode|packages/cli' packages/tui
