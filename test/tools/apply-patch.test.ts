...
test("apply_patch adds, updates, and deletes files inside writable roots", async () => {
  ...
  try {
    ...
    await expect(
      executor.execute(
        { id: "patch", name: "apply_patch", arguments: { patch } },
...
test("apply_patch rejects empty, move, and path escape patches as structured errors", async () => {
  ...
  try {
    ...
    await expect(
      executor.execute(
        { id: "empty", name: "apply_patch", arguments: { patch: empty } },
        { workspaceRoots: [workspace.root] },
      ),
    ).resolves.toMatchObject({ ok: false, error: { code: "invalid_patch" } })
    await expect(
      executor.execute(
        { id: "move", name: "apply_patch", arguments: { patch: move } },
        { workspaceRoots: [workspace.root] },
      ),
    ).resolves.toMatchObject({ ok: false, error: { code: "unsupported_patch" } })
    await expect(
      executor.execute(
        { id: "escape", name: "apply_patch", arguments: { patch: escape } },
        { workspaceRoots: [workspace.root] },
      ),
