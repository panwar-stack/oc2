...
import { MainAgent, buildAgentModelContext, createModelService, createSessionService, createToolExecutor, createToolRegistry, openOc2Database, resolveMainAgentProfile, defaultConfig, type ToolDefinition } from "../../src"
...
test("agent context includes system prompt, workspace roots, persisted messages, tools, and profile", () => {
  ...
  const session = sessions.createSession({ id: "session-1", workspaceRoots: [{ path: "/repo", readonly: false }], providerId: "fake", modelId: "test", agentId: "main" })
  ...
  const context = buildAgentModelContext({ session, messages: sessions.messages.listBySession(session.id), profile: resolveMainAgentProfile(defaultConfig), registry, config: defaultConfig })
...
test("agent loop persists final assistant text without tools", async () => {
  ...
  const session = sessions.createSession({ id: "session-1", workspaceRoots: [{ path: "/repo", readonly: false }], providerId: "fake", modelId: "test", agentId: "main" })
  const registry = createToolRegistry([testTool])
  const agent = new MainAgent({ sessions, registry, models: createModelService({ providers: [createScriptedModelProvider([simpleAssistantEvents])] }), tools: createToolExecutor({ registry }) })

  const result = await agent.run({ session, profile: resolveMainAgentProfile(defaultConfig), prompt: "hello", config: defaultConfig, signal: new AbortController().signal })
...
test("agent loop executes tool calls and persists tool results", async () => {
  ...
  const agent = new MainAgent({ sessions, registry, models: createModelService({ providers: [provider] }), tools: createToolExecutor({ registry }) })

  const result = await agent.run({ session, profile: resolveMainAgentProfile(defaultConfig), prompt: "use tool", config: defaultConfig, signal: new AbortController().signal })

  expect(result.status).toBe("completed")
  expect(result.toolCalls).toEqual([{ id: "tool-1", name: "answer", input: { value: "42" }, ok: true }])
  expect(sessions.messages.listBySession(session.id).map((message) => message.role)).toEqual(["user", "assistant", "tool", "assistant"])
