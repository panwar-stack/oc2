...
export const createApplyPatchTool = (): ToolDefinition<z.infer<typeof inputSchema>> => ({
  ...
  async execute(input, context) {
    ...
    for (const operation of operations) {
      resolved.push({
        operation,
        target: await resolveWorkspacePath(operation.path, context.workspaceRoots, {
          cwd: context.cwd,
          writable: true,
        }),
      })
...
const parsePatch = (patch: string): PatchOperation[] => {
  ...
  while (index < lines.length - 1) {
    ...
    if (line.startsWith("*** Move to:"))
      throw new ToolExecutionError({ code: "unsupported_patch", message: "Move patches are not supported" })
    if (!add && !update && !remove)
      throw new ToolExecutionError({ code: "invalid_patch", message: `Unexpected patch line: ${line}` })
    index += 1
    const body: string[] = []
    ...
    if (add)
      operations.push({
        type: "add",
        path: add[1] ?? "",
        lines: body.map((entry) => (entry.startsWith("+") ? entry.slice(1) : entry)),
      })
    if (update) operations.push({ type: "update", path: update[1] ?? "", lines: body })
    if (remove) operations.push({ type: "delete", path: remove[1] ?? "" })
  }
  if (operations.length === 0)
    throw new ToolExecutionError({ code: "invalid_patch", message: "Patch did not contain any file operations" })
  return operations
}
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (line.startsWith("-") && lines[index + 1]?.startsWith("+")) {
      ...
      if (!updated.includes(oldText))
        throw new ToolExecutionError({ code: "patch_no_match", message: `Patch line was not found: ${oldText}` })
